...
export const followInstruction = (
  ...
  stepList.forEach((step) => {
    ...
    if (count && initColumn && targetColumn) {
      ...
      const removedItems = initColumnItems.splice(
        initColumnItems.length - Number(count),
        initColumnItems.length
      );

      map.set(Number(initColumn), initColumnItems);
      if (isNewModel) {
        map.set(Number(targetColumn), [
          ...(map.get(Number(targetColumn)) ?? []),
          ...removedItems,
        ]);
      } else {
        map.set(Number(targetColumn), [
...
export const getTopContainers = (map: Map<number, string[]>) => {
  const entries = Array.from(map.entries()).sort(
    ([columnNumber1], [columnNumber2]) => columnNumber1 - columnNumber2
  );
